...
name: run-cortex
description: >
  Run, observe, and test the Cortex AI platform locally. Use when asked to start
  Cortex, run it under Aspire, read its logs/telemetry, exercise the chat
...
# Running & testing Cortex

Cortex is a **base platform** (`src/`, shipped as NuGet/npm packages) plus
**sample apps** (`samples/`). The runnable demo is the **sample app**, which
installs the Finance + Nutrition modules onto the platform.

> The platform host `src/Cortex.AppHost` runs `Cortex.Api`, which is a *bare
...
resources** (`cortex-ui` = domain UI, `cortex-admin-ui` = admin console), then
...
`cortex-sample`). Use that base URL for the API calls below.
...
$api = Start-Process dotnet -WorkingDirectory $bin -PassThru -ArgumentList @(
  "$PWD/$bin/Cortex.Sample.Host.dll",
  "--ConnectionStrings:cortex-platform=$($pg -f 'cortex_platform')",
  "--ConnectionStrings:cortex-audit=$($pg -f 'cortex_audit')",
...
# 4) Teardown
Stop-Process -Id $api.Id -Force; docker rm -f cortex-pg-test
```
...
```powershell
dotnet build Cortex.slnx; dotnet build samples/Cortex.Samples.slnx
dotnet test  Cortex.slnx; dotnet test  samples/Cortex.Samples.slnx
