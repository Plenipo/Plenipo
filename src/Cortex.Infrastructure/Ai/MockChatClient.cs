...
/// one), it emits a function call for an available tool, which Cortex dispatches through the genuine
/// pipeline — permission-filtered before the model sees it, audited, and (for side-effecting tools)
/// routed to human approval. This lets the zero-config demo showcase Cortex's signature capability
...
public sealed class MockChatClient : IChatClient
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        // First turn: if the user is asking for a tool, emit a function call. Cortex's function-invocation
    ...
    private static string BuildReply(IReadOnlyList<ChatMessage> messages, ChatOptions? options)
    {
        ...
        sb.Append("Hi! I'm the Cortex assistant running in mock mode — no AI provider is configured, ");
        ...
        if (toolNames.Length > 0)
        {
            ...
            sb.Append("(Cortex routes it through the same permission-checked, audited pipeline a real model would). ");
    ...
    private static string BuildToolSummary(IReadOnlyList<ChatMessage> messages, FunctionResultContent result)
    {
        ...
        sb.Append("Cortex ran it through the security pipeline (permission-checked before the model saw it, ");
