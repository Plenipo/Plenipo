...
/// A silent no-op until the "Email" section is configured; channel-synthesized addresses
/// (WhatsApp's *@whatsapp.channel) are skipped. Fired from the notifier's fan-out, which already
/// isolates failures.
/// </summary>
    ...
    PlatformDbContext db) : INotificationChannel
{
    public async Task SendAsync(Notification notification, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(email) || email.EndsWith("@whatsapp.channel", StringComparison.Ordinal))
        {
