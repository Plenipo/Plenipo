...
/// front-ends. Rather than expose the raw MAF agent (which would bypass Cortex's per-user tool
...
/// re-encodes the resulting events as AG-UI SSE frames. Any AG-UI client can therefore talk to Cortex.
/// </summary>
public static class AguiEndpoints
{
    ...
    private static async Task HandleAsync(
        ...
        Cortex.Infrastructure.Persistence.PlatformDbContext db,
    ...
    private static async Task<Guid> ResolveConversationIdAsync(
        Cortex.Infrastructure.Persistence.PlatformDbContext db, Guid tenantId, string threadId,
        CancellationToken cancellationToken)
    {
